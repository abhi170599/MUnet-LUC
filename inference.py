...
from DatasetPreparation import normalize
...
def get_model():
    ...
    model = load_model('model.h5',custom_objects={'weighted_binary_crossentropy':weighted_binary_crossentropy})
    return model
...
if __name__ == '__main__':
    ...
    img = tiff.imread('time1.tif')
    img = np.reshape(img,(img.shape[0],img.shape[1],1))
    img = np.concatenate([img]*8)
    img = img.transpose([1,2,0])
    img = normalize(img)   # make channels last
